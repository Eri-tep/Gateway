...
public:
  ...
  struct TelnetSession {
    ...
    uint32_t wizard_step_cooldown_until_ms{0}; // 스텝 간 잔여 패킷 무시 쿨다운 만료 시각
    uint32_t wizard_action_cooldown_until_ms{0}; // 조작 액션(ON->OFF 등) 간 버스트 패킷 무시 쿨다운 만료 시각
    uint8_t wizard_learned_devs[8]{0}; // 이번 세션에서 이미 완료된 기기 ID 목록 (중복 캡처 방지)
    uint8_t wizard_learned_count{0};
    ...
    void reset() {
      ...
      wizard_step_cooldown_until_ms = 0;
      memset(wizard_learned_devs, 0, sizeof(wizard_learned_devs));
      wizard_learned_count = 0;
